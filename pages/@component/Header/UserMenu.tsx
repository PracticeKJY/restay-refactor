...
const UserMenu = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: any) => {
      if (!userMenuOpenRef.current?.contains(e.target) && !userMenuRef.current?.contains(e.target)) {
        setIsMenuOpen(false);
      }
    };
    ...
    <>
      ...
      <div style={{ position: "relative" }}>
        <div className={styles.userMenuWrapper}>
          <div className={`${styles.userMenuText} ${session && !isRentModal ? styles.pointer : ""}`} onClick={onClick}>
            <div className={styles.hostingText}>{session ? `${session.user?.name}님 오늘은 어디로 떠나보실래요?` : "당신의 공간을 공유하세요"}</div>
          </div>
